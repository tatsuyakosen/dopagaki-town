...
test("500m LOW preset completes a real ten-minute match without performance decay", async ({ page }, testInfo) => {
  ...
  await page.goto("/");
  await page.getByLabel("CALL SIGN").fill("M2 Soak Runner");
  ...
  await testInfo.attach("m2-soak-report", {
    body: JSON.stringify(report, null, 2),
    contentType: "application/json",
  });
  process.stdout.write(`M2_SOAK_REPORT ${JSON.stringify(report)}\n`);
