...
import type {GeometryDownload} from "./geometry-source.js";
...
describe("bounded parallel geometry acquisition",()=>{
  it("never exceeds two downloads and preserves file ordering",async()=>{
    let active=0,peak=0;const download:GeometryDownload=async url=>{active++;peak=Math.max(peak,active);await new Promise(resolve=>setTimeout(resolve,5));active--;return {path:url,digest:sha256(url),bytes:10,hit:false};};
  ...
  it("waits for the bounded batch to settle before reporting a failure",async()=>{
    let ended=false;const download:GeometryDownload=async url=>{if(url.endsWith("0"))throw new Error("SOURCE_TIMEOUT");await new Promise(resolve=>setTimeout(resolve,5));ended=true;return {path:url,digest:sha256(url),bytes:10,hit:false};};
