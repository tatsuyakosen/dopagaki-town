...
export async function build(catalog:Catalog,lod:1|2,progress:(value:Progress)=>void=()=>{},tile?:AreaTile){
  ...
  const origin=tile??catalog;
  const bounds=tile?tileBounds(tile):undefined;
  const manifest=await convert(paths,metadata,[origin.latitude,origin.longitude,0],tile?500:125,[0,1,0],lod,`${catalog.city} / 選択した街`,bounds);
  ...
  let terrainStats={tiles:0,bytes:0,drapedRoads:0,alignedBuildings:0};
  ...
  return {manifest,stats:{sourceBytes:received,sourceCacheHits:hits,vertices,meshes:manifest.meshes.length,manifestBytes:bytes,terrainTiles:terrainStats.tiles,terrainBytes:terrainStats.bytes,drapedRoads:terrainStats.drapedRoads,alignedBuildings:terrainStats.alignedBuildings,workerSeconds:(performance.now()-started)/1000}};
}
