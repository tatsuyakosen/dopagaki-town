...
export function drawMinimap(canvas:HTMLCanvasElement,x:number,z:number,ready:ReadonlySet<string>,failed:ReadonlySet<string>):void{
  const ctx=canvas.getContext("2d");if(!ctx)return;
  const size=canvas.width,scale=size/1000;ctx.clearRect(0,0,size,size);
  for(let tx=-2;tx<=2;tx++)for(let tz=-2;tz<=2;tz++){
    const bounds=tileBounds({x:tx,z:tz}),key=tileKey(tx,tz);ctx.fillStyle=ready.has(key)?"#416977":failed.has(key)?"#684139":"#202c3c";
