...
async function readManifest():Promise<CityManifest> {
  ...
  const response=await fetch("/city-data/umeda-block.json",{credentials:"omit",cache:"no-store",redirect:"error",signal:AbortSignal.timeout(15_000)});
...
async function boot():Promise<void> {
  ...
  try{
    ...
    engine=new Engine(canvas,true,{preserveDrawingBuffer:false,stencil:true});engine.setHardwareScalingLevel(1.5);
    ...
    world=createWalkWorld(scene,manifest,shadows);const activeWorld=world;
    ...
    scene.onBeforeRenderObservable.add(()=>{
      ...
      if(!paused){
        ...
        if(player.position.y< -200 || Math.abs(player.position.x)>manifest.playableHalfSize+2 || Math.abs(player.position.z)>manifest.playableHalfSize+2)reset();
      }
      ...
      for(const offset of [Vector3.Zero(),new Vector3(.25,0,0),new Vector3(-.25,0,0),new Vector3(0,.2,0),new Vector3(0,-.2,0)]){
        const pick=scene.pickWithRay(new Ray(target.add(offset),direction,5.5),mesh=>activeWorld.colliders.has(mesh as Mesh)&&mesh.isEnabled());
    ...
    credits(manifest);hud.hidden=false;start.hidden=false;
    ...
    message.textContent="未配置・形式不正・容量超過・安全な開始地点がない、または3D描画に非対応です。手順書に沿ってデータを準備してください。架空の街への自動置換は行いません。";
