...
梅田の隣接2街区で構築と境界越えをCPU衝突テストで検証しました。AIのLOD計画は任意接続で、未設定時は「AI未接続」と表示します。
1km全域、全国すべての地点、実機FPS、実AIによる構築はまだ検証を完了していません。
...
地図で選択 → 「この場所で歩く」で構築後に自動入場します。有効な完成データは再起動後も直接開けます。
元データは同梱せず、公式PLATEAU APIの建物・道路と国土地理院のDEMを取得します。標高ゼロの地物はDEMに合わせた補正だと明示します。Match Server・AI・APIキーなしで自動構築できます。
地図の初回JavaScriptは約48KB gzip。変換は別プロセスで行い、描画は最大5街区・合計10万〜18万頂点に制限します。取得済み街区のみ境界を開き、状態をミニマップに表示します。
初回は配布元通信で長く待つ場合があります。今回の梅田では確認・取得・構築の合計約92秒、元ファイル取得済みなら構築約2.8秒を観測しました。画面描画時間やFPSではありません。
