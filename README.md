...
梅田は最初の検証地点です。地域選択・AIによる自動構築はまだ実装していません。
第一段階の完了条件と実装との差分は [エリア選択から街で遊ぶまで](docs/AREA_TO_PLAY.md) に記録しています。

## 街歩きプロトタイプ（実都市データ準備中）

既存の対戦モードを維持し、独立した `/walk.html` を追加しています。
`npm ci` → `npm run dev:client` で起動し、街歩きはMatch Server・AI・APIキーを必要としません。
実都市データは同梱していません。データなしの通常画面は未配置を案内し、架空都市へ自動置換しません。
